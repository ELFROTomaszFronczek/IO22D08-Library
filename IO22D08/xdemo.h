  ...
  if (USE_LCD) {
    ...
    if (currentStep == 0)
    {
      ...
      else
      {
        ...
        Serial.print("ADC VAL: "); Serial.println(rawADC );
        Serial.print("Volts from ADC: "); Serial.print(currentRawVoltage);
        Serial.print("(int) "); Serial.print(currentRawVoltage / 100.0); Serial.println("V");
        Serial.print("V for user: "); Serial.print(currentVoltage);
        Serial.print("(int) "); Serial.print(currentVoltage / 100.0); Serial.println("V  !!!!");
        Serial.println();
        setLCDbyInt(currentVoltage,true);
        setLCDdigit(3,'v');
      }
      i++;
      if (i == 1) {
